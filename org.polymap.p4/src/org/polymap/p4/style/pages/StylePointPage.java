 ...
 *
 */
public class StylePointPage
        extends AbstractStylePage<StylePointDao> {
     ...
     */
    @Override
    public StylePointDao createEmptyDao() {
        return new StylePointDao();
    }
    ...
    private void createStyleTabItemForPoint( IFormPageSite site ) {
        ...
        site.newFormField( new BeanPropertyAdapter( getDao(), StylePointDao.MARKER_STROKE_SIZE ) ).label
                .put( "Marker border size" ).field.put( markerStrokeSizeField ).tooltip.put( "" ).create();
        markerStrokeColorField = new ColorFormField();
        site.newFormField( new BeanPropertyAdapter( getDao(), StylePointDao.MARKER_STROKE_COLOR ) ).label
